      ...
      <div class="order-top-box">
        ...
        <div class="order-title-item">
          ...
          <div class="order-right-title">
            {{
              deliveryType == "buy" ? unit_fee * buyPrice : unit_fee * sellPrice
            }}
          </div>
        </div>
        <div class="order-title-item">
...
export default {
  ...
  methods: {
    ...
    async getDeliveryInfo() {
      Axios.getFutures({
        ...
        if (res.data.para.length > 0) {
          ...
          this.buyPrice = this.buy_min;
          this.sellPrice = this.buy_min;
          let deliveryUnit = "";
    ...
    //买入数量失去焦点
    blurBuyNum() {
      if (this.buyPrice <= this.buy_min) {
        this.buyPrice = this.buy_min;
      }
    },
    //做空数量失去焦点
    blurSellNum() {
      if (this.sellPrice <= this.buy_min) {
        this.sellPrice = this.buy_min;
      }
    },
    ...
    openOrder() {
      Axios.futuresOpen({
        ...
        if (res.code == 0) {
          ...
          this.currencyPrice = this.currencyPrice;
    ...
    buyCurrency() {
      ...
      this.nowType = "buy";
      this.currencyPrice = this.buyPrice;
      this.buyAndSellData = {
        symbol: this.currencyData.symbol,
        session_token: this.sessionToken,
        direction: this.nowType,
      ...
      // this.openOrder(this.nowType, this.currencyPrice);
    },
    //卖出
    sellCurrency() {
      if (!this.sellPrice) {
      ...
      this.nowType = "sell";
      this.currencyPrice = this.sellPrice;
      this.buyAndSellData = {
        symbol: this.currencyData.symbol,
        session_token: this.sessionToken,
        direction: this.nowType,
    ...
    fastFun(item) {
      if (this.deliveryType == "buy") {
        this.buyPrice = this.volume * (item / 100);
      } else {
        this.sellPrice = this.volume * (item / 100);
      }
    },
