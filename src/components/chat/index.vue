<template>
  <div class="chat" v-loading="loading">
          ...
          <li v-for="(item, index) in chatList" :key="item.id">
            ...
            >
              ...
              <template v-if="item.send_receive === 'receive'">
                <img src="./responser.png" class="avatar" />
                <div
    ...
    <!-- 发送消息 -->
    <div class="input-box">
      <img src="./photo.png" class="w-72 h-72" @click="handleChoiceFile" />
      <input
        ref="upload"
        type="file"
        hidden
...
export default {
  ...
  methods: {
    ...
    scrollToBottom() {
      this.$nextTick(() => {
        console.log("到顶部的距离不生效", this.$refs.scrollInner.clientHeight);
